...
class AlgorithmClienteServidor(Model):
    ...
    def DLOCK(self):
        print(f"[t={self.clock}] Nodo {self.id} ejecuta DLOCK solicitando acceso al recurso.")
        servidor = self.neighbors[0]
        return Event("SOLICITUD", self.clock + 1.0, servidor, self.id)
    
    def DUNLOCK(self):
        print(f"[t={self.clock}] Nodo {self.id} ejecuta DUNLOCK liberando el recurso.")
        servidor = self.neighbors[0]
        return Event("LIBERA", self.clock + 1.0, servidor, self.id)
        ...
        if event.getName() == "INICIA":
            if self.id != 1:
                ...
                if lanzar_moneda == elegir:
                    ...
                    evento_solicitud = self.DLOCK()
                    self.transmit(evento_solicitud)
                else:
            ...
            evento_liberar = self.DUNLOCK()
            self.transmit(evento_liberar)
